...
Public Class Form2
    ...
    Dim map(2000, 2000, 2) As Integer
    Dim Grid As Form
    ...
    Private Sub Timer1_Tick(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Timer1.Tick
        ...
        '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        y_robot = y_alt + (ticks / 24) * Math.Cos((drive_angle) * Math.PI / 180)
        x_robot = x_alt + (ticks / 24) * Math.Sin((drive_angle) * Math.PI / 180)
    ...
    Private Sub Generate_grid(ByVal Dist() As Single, ByVal x As Integer, ByVal y As Integer)
        ...
        Dim stift2 As New Pen(Color.FromArgb(128, Color.Red), 3)
        Dim stift3 As New Pen(Color.Black, 3)
        Dim i, x_map, y_map As Integer
        Dim ky, kx, occupancy As Single
        Dim sensor_angle() As Integer = {0, 45, 71, 90, 109, 135, 180}
        Dim xd, yd, x_vision, y_vision As Integer
        Dim stepwidth As Single
        Dim evidence As Double
        Dim stepsign As Integer
        ...
        For i = 0 To 6
            ...
            xd = Math.Cos(sensor_angle(i) * Math.PI / 180) * (Dist(i) - 0) ' Polar-Koordinaten bezogen auf die Sichtachsen des Roboters
            yd = Math.Sin(sensor_angle(i) * Math.PI / 180) * (Dist(i) - 0)

            Coordinate = rotate_xy(xd, yd, drive_angle)   ' Absolute Koordinaten im Raum

            x_vision = Coordinate(0)
            y_vision = Coordinate(1)

            If Dist(i) <= 80 Then


                'Ab hier alle auf der Sichtlinie liegenden Punkte bis zum Hindernis oder maximal bis zur "Sichtgrenze" nullen


            If Math.Abs(x_vision) > Math.Abs(y_vision) Then
                stepwidth = Math.Abs(y_vision / x_vision) 'Division durch 0?
                'If (y_vision < 0) And (x_vision < 0) Then stepwidth = stepwidth * -1
                ...
                If x_vision <= 0 Then
                    While kx > x_vision
                            ...
                            If map(kx + x, 600 - (ky + y), 0) < 256 Then

                                map(kx + x, 600 - (ky + y), 0) += 1
                            End If
            ...
            If Math.Abs(x_vision) <= Math.Abs(y_vision) Then
                stepwidth = Math.Abs(x_vision / y_vision)
                'If (y_vision < 0) And (x_vision < 0) Then stepwidth = stepwidth * -1
                'TextBox3.Text += stepwidth.ToString + " "

                kx = 0
                ky = 0
                If y_vision >= 0 Then
                    For ky = 0 To y_vision
                ...
                If y_vision < 0 Then
                    While ky > y_vision
                            ...
                            If x_vision > 0 Then
                                kx = kx + stepwidth ' was passiert bei Integer + Single
                            Else
                                kx = kx - stepwidth
                            End If
                            ky = ky - 1
                ...
                If (Dist(i) < 79) Then
                    If map(Coordinate(0) + x, 600 - (Coordinate(1) + y), 1) < 256 Then
                        map(Coordinate(0) + x, 600 - (Coordinate(1) + y), 1) += 1 ' so oft Hindernis an dieser Stelle gesehen
                    End If
                    'TextBox3.Text += map(Coordinate(0) + x, 600 - (Coordinate(1) + y), 0).ToString + " "
                End If
        ...
        'Nach vielen Messungen erscheinen keine neuen Hindernisse mehr

        For y_map = 0 To 600
            For x_map = 0 To 600
                evidence = map(x_map, y_map, 1) / map(x_map, y_map, 0)
                If (evidence > 0.5) Then
                    z2.DrawEllipse(stift2, x_map, y_map, 2, 2)
                    'TextBox3.Text += x_map.ToString + "." + y_map.ToString + "." + evidence.ToString + ":"
                End If
                'End If
                'End If
